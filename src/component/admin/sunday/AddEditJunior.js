...
import { firebaseDB, firebaseJunior } from "../../../firebase";

class AddEditJunior extends Component {
  state = {
    lessonId: "",
    isPage: "",
    formSuccess: "",
    ...
    if (!lessonID) {
      ...
      firebaseDB
        ...
        .then(snapshot => {
          ...
          this.updateField(lessonData, lessonID, "Edit Junior Lesson");
        });
    }
  }
  ...
  submitForm = event => {
    ...
    if (validForm) {
      if (this.state.isPage === "Edit Junior Lesson") {
        let lessonDetails = {
        ...
        firebaseJunior
          ...
          .then(snapshot => {
            if (snapshot.val() === null) {
              firebaseJunior.child(submitData.language).push(lessonDetails);
