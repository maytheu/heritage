...
import { firebaseDB, firebaseAdult } from "../../../firebase";

class AddeditSunday extends Component {
  state = {
    lessonId: "",
    lesson: [],
    isPage: "",
  ...
  componentDidMount() {
    ...
    if (!lessonID) {
      ...
      firebaseDB
        .ref(`adult/${this.props.lang.lang}/${lessonID}`)
        .once("value")
        .then(snapshot => {
          ...
          this.updateField(lessonData, editorState, lessonID, "Edit Lesson");
        });
    }
  }
  ...
  submitForm = event => {
    ...
    if (validForm) {
      if (this.state.isPage === "Edit Lesson") {
        let lessonDetails = {
        ...
        firebaseDB
          .ref(`adult/${this.props.lang.lang}/${this.state.lessonId}`)
        ...
        firebaseAdult
          ...
          .then(snapshot => {
            if (snapshot.val() === null) {
              firebaseAdult.child(submitData.language).push(lessonDetails);
...
export default connect(mapStateToProps)(AddeditSunday);
